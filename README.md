...
第 4 阶段不是聊天摘要：必须把完整图片计划、逐图规格、文字、几何资产和基础信息摘要写入原有的 `<路线名称>-路线核对单.md`。写入后重新读取并校验：

```bash
python3 scripts/validate_stage4_review.py route-work/route-路线核对单.md
```

如果核对单不存在，应先根据已经确认的信息重建前置章节。只有文件校验通过并向用户展示文件路径及完整第 4 阶段正文后，才能请求 `生图画面规格确认无误`。

在步骤 5 摘要之前，还必须完成一次路线文案调研：查询适合时间/季节、适合人群、装备、天气路况、补给、交通、开放状态和安全注意事项，记录来源与查询日期，筛选后更新逐图文字。用户确认 `路线文案与注意事项确认无误` 后，才生成基础信息摘要。来源链接保留在核对单中，图片只放简洁、已确认的文案。
